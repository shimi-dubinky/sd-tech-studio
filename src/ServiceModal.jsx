...
import { X, CheckCircle2, MessageCircle, Zap } from 'lucide-react';

const phone = "0503413689";
const baseWhatsapp = `https://wa.me/972${phone.substring(1)}`;

const ServiceModal = ({ isOpen, onClose, service }) => {
  // תיקון כפתור "חזור" במובייל - מונע יציאה מהאתר
  useEffect(() => {
    if (isOpen) {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-3xl bg-zinc-950/60 backdrop-blur-3xl border border-teal-500/20 p-8 md:p-14 rounded-[2.5rem] shadow-2xl overflow-y-auto max-h-[90vh]"
            dir="rtl"
          >
            ...
            <div className="space-y-6 mb-12">
              <h4 className="text-white font-bold text-xl border-b border-white/5 pb-3 italic">מה כלול בפתרון?</h4>
              <div className="grid grid-cols-1 gap-4">
                {service.examples.map((item, idx) => (
                  <div key={idx} className="flex items-start gap-4 p-5 bg-white/5 rounded-2xl border border-white/5 hover:border-teal-500/30 transition-all">
            ...
            >
              <MessageCircle size={22} /> בוא נדבר על הפתרון <Zap size={18} className="animate-pulse" />
            </motion.a>
          </motion.div>
        </div>
      )}
