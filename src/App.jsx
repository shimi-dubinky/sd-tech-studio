  ...
  const services = [
    { 
      ...
      examples: [
        ...
        { name: "Single Page Applications", desc: "ממשקי משתמש מהירים ללא ריענון דף, מבוססי Vite & React לחוויית גלישה רציפה." },
    ...
    { 
      ...
      examples: [
        ...
        { name: "Data Processing Bots", desc: "עיבוד וניתוח קבצי דאטה גדולים והפקת תובנות אוטומטיות ישירות למייל." }
      ]
    },
    { 
      ...
      examples: [
        ...
        { name: "חילוץ מידע ממסמכים", desc: "קריאה אוטומטית של קבצי PDF והפיכתם לנתונים דיגיטליים מסודרים בתוך שניות." }
      ]
    }
  ];
    ...
    <div className="min-h-screen bg-zinc-950 text-white font-sans overflow-x-hidden selection:bg-teal-500/30" dir="rtl">
      {init && (
        <Particles id="tsparticles" className="absolute inset-0 z-0" options={{
          fpsLimit: 60, particles: {
            ...
            move: { enable: true, speed: 0.8 }, number: { value: 40 }, opacity: { value: 0.2 }, size: { value: 1 }
          }
        }} />
      )}

      {/* Blobs לאפקט במובייל */}
      <div className="absolute top-20 left-10 w-72 h-72 bg-teal-600/10 rounded-full blur-3xl animate-blob"></div>
      <div className="absolute bottom-40 right-10 w-80 h-80 bg-electricBlue-600/10 rounded-full blur-3xl animate-blob" style={{animationDelay: '2s'}}></div>
      ...
      <nav className="fixed top-0 w-full z-50 backdrop-blur-xl bg-zinc-950/70 border-b border-teal-500/10 py-5 px-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="text-2xl font-black tracking-tighter uppercase"><span className="bg-gradient-to-r from-teal-400 to-electricBlue-400 bg-clip-text text-transparent">SD Tech</span></div>
          <a href={baseWhatsapp} target="_blank" rel="noopener noreferrer" className="bg-gradient-to-r from-teal-600 to-electricBlue-600 px-6 py-2 rounded-full text-xs font-bold">שיחת אפיון</a>
        </div>
      </nav>

      <motion.section style={{ opacity: heroOpacity }} className="relative pt-48 pb-32 px-6 text-center z-10">
        <Tilt tiltMaxAngleX={5} tiltMaxAngleY={5} className="max-w-5xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-teal-950/50 border border-teal-500/30 text-teal-300 text-[10px] font-bold tracking-widest uppercase mb-10 backdrop-blur-md">
            <Star size={12} className="animate-spin" /> מבצע השקה לזמן מוגבל
          </div>
          <h1 className="text-5xl md:text-8xl font-black mb-8 leading-[1.1] tracking-tighter">
            הנדסת תוכנה.<br />
            <span className="bg-gradient-to-r from-teal-400 via-electricBlue-400 to-fieryOrange-400 bg-clip-text text-transparent drop-shadow-[0_2px_15px_rgba(0,0,0,0.8)]">
             פתרון טכנולוגי שלם.
            </span>
          </h1>
          <p className="text-zinc-400 text-lg md:text-2xl max-w-3xl mx-auto mb-12 font-light">
            <span className="text-white font-bold">שמעון דובינסקי</span> // מומחה פיתוח המשלב חוויית משתמש פרימיום עם אוטומציה חכמה.
          </p>
          <a href="#pricing" className="bg-white text-black px-10 py-5 rounded-2xl font-bold text-lg hover:bg-teal-50 transition-all inline-flex items-center gap-3">גלה את הפתרונות <ArrowLeft /></a>
        </Tilt>
      </motion.section>
          ...
          {services.map((s, i) => (
            <Tilt key={i} tiltMaxAngleX={8} tiltMaxAngleY={8} scale={1.02}>
              <div onClick={() => setSelectedService(s)} className="h-full p-10 rounded-[2.5rem] border border-teal-500/10 bg-zinc-900/40 backdrop-blur-xl hover:border-electricBlue-500/40 transition-all cursor-pointer group shadow-2xl">
                ...
                <div className="flex items-center gap-2 text-electricBlue-400 font-bold text-xs tracking-widest uppercase">חקור את המפרט <ArrowLeft size={16} /></div>
              </div>
            </Tilt>
          ))}
        </div>
          ...
          {products.map((p, i) => {
            ...
            return (
              <div key={i} className={`relative p-10 rounded-[3rem] bg-gradient-to-br ${p.gradient} backdrop-blur-2xl flex flex-col border ${p.recommended ? 'border-electricBlue-500/50 shadow-2xl md:scale-105 z-20' : 'border-teal-500/10'} transition-all`}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 mb-10">
                  <span className="text-5xl font-black">₪{p.price}</span>
                  <span className="text-xl text-zinc-500 line-through opacity-50">₪{p.oldPrice}</span>
                </div>
      ...
      <footer className="py-20 text-center relative z-10 border-t border-teal-900/30">
        <p className="text-xs font-mono text-zinc-600 uppercase tracking-[0.3em]">© {new Date().getFullYear()} Shimon Dubinsky // Engineered Solutions</p>
      </footer>
